...
const ProvimentosDao = app_require('routes/v1/provimentos/provimentos.dao');

module.exports.install = (router) => {
    router.get('/provimentos', getProvimentos);
};

async function getProvimentos(ctx, next) {
    ...
    const dao = new ProvimentosDao(ctx.db);
